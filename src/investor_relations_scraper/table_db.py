...
class DuckDBManager:
    ...
    Uses short-lived connections (open / execute / close) per operation so
    that the .duckdb file is never held open exclusively between calls.
    This avoids the 'Conflicting lock' error when the app is restarted or
    ...
    def __init__(self, db_path: Optional[str] = None, processed_dir: Optional[str] = None):
        ...
        # Bootstrap: create the metadata catalog if it doesn't exist yet
    ...
    @contextmanager
    def _connect(self):
        """Open a short-lived DuckDB connection, yield it, then close."""
    ...
    def sync_csvs(self):
        ...
        with self._connect() as con:
            res = con.execute("SELECT csv_filename FROM _table_catalog").fetchall()
            ...
            for csv_path in csv_files:
                ...
                try:
                    con.execute(f"CREATE OR REPLACE TABLE {db_table_name} AS SELECT * FROM read_csv_auto('{csv_path}')")

                    schema_res = con.execute(f"DESCRIBE {db_table_name}").fetchall()
    ...
    def reload_csv(self, csv_filename: str):
        ...
        with self._connect() as con:
            try:
                ...
                con.execute("DELETE FROM _table_catalog WHERE csv_filename = ?", (csv_filename,))
                con.execute(
                    "INSERT INTO _table_catalog VALUES (?, ?, ?, ?)",
    ...
    def get_catalog_for_sources(self, source_pdfs: List[str]) -> List[Dict[str, Any]]:
        ...
        with self._connect() as con:
            res = con.execute(query, source_pdfs).fetchall()
    ...
    def close(self):
        """No-op: connections are now short-lived and closed after each operation."""
        pass
